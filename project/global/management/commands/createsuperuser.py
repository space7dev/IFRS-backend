"""
This custom 'createsuperuser' command serves to override the django original to
add custom user roles to the process.

Line 193 is the only difference with the original command.
"""
import getpass
import os
import sys
    ...
    def handle(self, *args, **options):
        ...
        try:
            if options['interactive']:
                ...
                # Use password from input, if provided.
                if password is None:
                    raise CommandError(
                        'You must use --%s with --noinput.' % self.PASSWORD_FIELD)
            ...
            user_data["role"] = self.UserModel.ADMIN
